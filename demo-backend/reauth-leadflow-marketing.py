"""
Re-authorize Google OAuth for Leadflow Marketing project.
Run: doppler run --project fulfillment-automation --config dev_claudio -- python3 reauth-leadflow-marketing.py
...
result = subprocess.run(
    ["doppler", "secrets", "set", "LEADFLOW_MARKETING_GOOGLE_OAUTH_TOKEN",
...
if result.returncode == 0:
    print("\n--- Leadflow Marketing OAuth Token in Doppler gespeichert! ---\n")
else:
    print("\nFehler beim Speichern in Doppler:")
    print(result.stderr)
    print("\nManuell speichern mit diesem JSON:\n")
    print(token_json)
    print()
