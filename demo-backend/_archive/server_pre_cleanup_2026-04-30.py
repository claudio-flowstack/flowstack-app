...
═══════════════════════════════════════════════════════════════════════════════
WICHTIG: Diese Datei enthält V1 (LIVE) UND V2 (Archiv) Handler!
  • V1 Handler: node_id wie "is02", "ca07", "kc05" — OHNE Prefix → LIVE für Demo
  • V2 Handler: node_id wie "v2-create-lead", "v2-st01", "v2-cc01" → ARCHIV
  • V3 läuft separat in v3_server.py + v3/phases/*.py — NICHT in dieser Datei
═══════════════════════════════════════════════════════════════════════════════
...
async def close_api_v2(method: str, path: str, data: Optional[dict] = None) -> dict:
    """Close CRM API call für V2 Org (Leadflow Marketing)."""
    resp = await _http.request(
        method,
        f"https://api.close.com/api/v1{path}",
        json=data,
...
# CSS für generierte Docs (aus create-staged-docs.py)
V2_DOC_STYLE = """
<style>
  body {
...
def _load_framework(doc_number: int) -> str:
    """Lade Framework-Datei für ein Dokument (01-12)."""
...
async def generate_v2_document(
    ...
    Args:
        ...
        doc_name: Name des Dokuments (für Google Doc Titel)
    ...
    # Kontext für den Prompt zusammenbauen
    ...
    system_prompt = f"""Du bist ein Senior Marketing-Stratege und Consumer-Psychologe für eine Performance-Marketing-Agentur.
Du erstellst Elite-Level Strategie-Dokumente für Recruiting-Kampagnen.
...
PSYCHOLOGIE-REGELN:
- Loss Aversion: Verluste ~2x stärker als Gewinne. Schmerzpunkte IMMER vor Benefits.
- Social Proof: Konkrete (fiktive) Personas, Zahlen, "Dein Team" statt "Unser Team".
- Identity Play: Selbstbild spiegeln ("Du bist jemand, der..."), Anti-Identität nutzen.
- Cognitive Ease: Kurze Sätze. Einfache Wörter. Zeilenumbruch nach JEDEM Gedanken.
...
Alle Inhalte müssen spezifisch für das Unternehmen sein - KEINE generischen Platzhalter.
Sprache: Deutsch.

{framework}"""

    user_prompt = f"""Erstelle das Dokument "{doc_name}" für folgendes Projekt:

{context_text}

Generiere den KOMPLETTEN HTML-Body-Inhalt. Jede Section muss ausgefüllt sein mit spezifischen, maßgeschneiderten Inhalten.
Keine Platzhalter wie [hier einfügen] - alles muss fertig und professionell sein.
Qualität auf dem Niveau einer Top-Agentur - nicht generisch, nicht austauschbar."""
...
async def execute_node(body: dict):
    ...
    try:
        ...
        # ═══════════════════════════════════════════════════════════════════
        # V2 ARCHIV — AI Content Pipeline (Close V2 + Airtable + OpenRouter)
        # NICHT für Demo nutzen. Node-IDs mit "v2-" Prefix.
        # ═══════════════════════════════════════════════════════════════════
            ...
            if existing_lead and existing_opp:
                ...
                log.warning("Close nicht konfiguriert - Lead übersprungen")
        ...
        # ══════════════════════════════════════════════════════════════════
        # V2 Airtable + Extraktion - Neue Nodes für Lego-Baustein-System
        # ══════════════════════════════════════════════════════════════════
            ...
            if AIRTABLE_BASE_ID and AIRTABLE_API_KEY:
                ...
                log.warning("Airtable nicht konfiguriert - Client-Record übersprungen")
            ...
            # Quality Gate: Prüfen ob die wichtigsten Kategorien vorhanden sind
            ...
            if client_id and AIRTABLE_BASE_ID:
                ...
                # Client Status → "Strategie" (Extraktion abgeschlossen, bereit für AI)
        ...
        # Close CRM Updates (Leadflow Marketing Org)
            ...
            if list_id_st:
                if task_ids_st.get("bausteine_review"):
                    log.info("V2: Bausteine-Review Task bleibt offen - Mensch muss prüfen und abhaken.")
                # Task: V2 Copy-Review
            ...
            if list_id_cc:
                if task_ids_cc.get("v2_copy_review"):
                    ...
                    log.info("V2: Copy-Review Task bleibt offen - Mensch muss prüfen und abhaken.")
...
4. Hooks: Max 125 Zeichen, emotional triggern, Loss Aversion > Benefits
5. CTAs müssen niedrigschwellig sein: "In 60 Sekunden", "Kein Lebenslauf", "Unverbindlich"
6. VERBOTENE Wörter: "Rockstar", "Ninja", "Guru", "Dynamisches Team", "Flache Hierarchien"
7. Fachwörter: Echte Begriffe die die Zielgruppe TÄGLICH nutzt
8. Psychologie-Felder: Denke wie ein Therapeut - was FÜHLT die Person wirklich?
9. Trigger-Events: Konkrete Situationen die den Wechsel auslösen
10. Kernbotschaft: MUSS in einem einzigen Satz die gesamte Kampagne kommunizieren
